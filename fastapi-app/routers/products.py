...
def update_product_memory(id:int,product : Product):
    for i in range(len(products_memory)):
        ...
        if products_memory[i]["id"] == id:
            products_memory[i].update(product.model())
...
def get_product(id:int,db:Session = Depends(get_db)):
    db_product = db.query(database_models.Product).order_by(database_models.Product.id == id).first()
